...
frame="${1:?usage: test-installed-desktop-frame.sh FRAME.png}"
checker="$repo_root/build/test-installed-desktop-frame/check-installed-desktop-frame"
...
if [[ "${GUIASSERT_DEV_SHELL:-0}" != 1 ]]; then
  ...
  if [[ -z "$nix_bin" ]]; then
    echo "nix is required to enter GuiAssert's desktop-frame environment" >&2
    exit 2
  fi
...
nim c --hints:off --warnings:off \
  ...
  --out:"$checker" \
  "$repo_root/tests/test_installed_desktop_frame.nim"
"$checker" "$frame"
